<template>
  <div class="v_slider">
    ...
    <button v-if="nav" class="v_slider__prev" v-html="prevNav" @click="prevSlide">
    </button>
    <button v-if="nav" class="v_slider__next" v-html="nextNav" @click="nextSlide">
    </button>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.$nextTick(function () {
      ...
      if (this.settings.loop) {
        const lastSlide = this.el.track.getElementsByClassName('v_slider__item');
        for (let i = 0; i < this.numItemReal; ++i) {
          const clone = lastSlide[i].cloneNode(true);
          clone.classList.add('cloned');
          this.el.track.insertBefore(clone, this.el.slides[this.numItemReal - 1 + i].nextSibling);
        }
        for (let i = this.numItemReal; i > 0; --i) {
          const clone = lastSlide[this.numItemReal - 1].cloneNode(true);
          clone.classList.add('cloned');
  ...
  methods: {
      ...
      if (this.breakpoints) {
        this.breakpoints.forEach((width) => {
          if (width <= this.width.document) {
            for (const key in this.responsive[width]) {
    ...
    addActiveClass(value) {
      ...
      const superClass = value + Math.floor(((this.settings.items / 2)));
    ...
    setDot(n) {
      ...
      } else if (n == this.numDot) {
        const remainder = this.numItemReal % this.settings.items;
        if (remainder == 0) {
          this.setSlide((n - 1) * (this.settings.items));
        } else {
    ...
    handleMouseMove(e) {
      const positionX = ('ontouchstart' in window) ? e.touches[0].clientX : e.clientX;
      const positionY = ('ontouchstart' in window) ? e.touches[0].clientY : e.clientY;
  ...
  updated() {
    this.$nextTick(function () {
      ...
      if (this.settings.loop) {
        ...
        if (remainder == 0) {
          this.numDotActive = Math.ceil(this.itemActive / this.settings.items + 0.1);
        } else if (this.itemActive >= this.numItemReal - this.settings.items) {
